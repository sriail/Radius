...
const bareServer = createBareServer("/bare/", {
    connectionLimiter: {
        // Increase limits to allow for authentication flows which may require multiple requests
        maxConnectionsPerIP: parseInt(process.env.BARE_MAX_CONNECTIONS_PER_IP as string) || 500,
        windowDuration: parseInt(process.env.BARE_WINDOW_DURATION as string) || 60, // Longer window for auth flows
        blockDuration: parseInt(process.env.BARE_BLOCK_DURATION as string) || 30,  // Shorter block duration
    },
    // Don't filter any remote URLs to allow connections to authentication services
    // This ensures reCAPTCHA, Cloudflare, and Google Sign-in can function properly
